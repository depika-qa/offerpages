import payment_methods_check from '../../support/commands'
describe('Rs.1188 offer page ', () => {
    ...
    it.only('Before login price check', () => {
        ...
        cy.get('.mg-info')
         ...
         //cheking payment methods 
        cy.payment_methods_check()    
...
it('Homepage check', () => {
    ...
    cy.get('.header__menu__list').should('contain', 'Magazines').should('be.visible')
    ...
    cy.get('.mbanner__cn__title').should('contain', 'Get 1 Year of Magzter GOLD access at an unbelievable offer price').should('be.visible')
    ...
    cy.get('strike').should('contain', '₹4,000').and('be.visible')
    cy.get('.mbanner__cn__priceinfo__offer').should('contain', '₹2,000').and('be.visible')
    cy.get('.mbanner__cn__savepercent').should('contain', 'Save 50%').should('be.visible')
...
it('After login as normal user', () => {
    ...
    cy.get('.form__control').type('65ffa56e-f39a-4bfd-a9d8-3a51d3fcf3d0@mailslurp.com')
    cy.get('button').contains('Continue').click({ timeout: 10000 })
    cy.get('[type="password"]').type('1234')
    cy.get('button').contains('Continue').click({ timeout: 10000 })
    cy.wait(5000)
    cy.url().should('eq', 'https://www.magzter.com/magztergold/1year-offer', { timeout: 10000 })
